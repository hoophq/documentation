...
<Note>
  Already installed? Once you've authenticated against a gateway, you can keep the CLI in sync with `hoop upgrade` — it downloads the matching release, installs it under `$HOME/.hoop/versions/`, and manages multiple versions side-by-side. See [hoop upgrade](/clients/cli-upgrade) for details.
</Note>
