...
For databases and other TCP services, `hsh` also ships the
[Hoop Tunnel](/clients/hsh-tunnel) — a background daemon that makes
every connection reachable by name, e.g. `psql -h pg-prod.hoop`.
...
<Tabs>
  <Tab title="MacOS">
    ...
    ```bash
      brew trust --formula hoophq/brew/hoop
    ...
    <Note>
      The formula also installs the `hsh-tunneld` daemon for the [Hoop Tunnel](/clients/hsh-tunnel). To use the tunnel, register the daemon as a system service once: `sudo hsh-tunneld install` — see the [tunnel installation steps](/clients/hsh-tunnel#installation).
    </Note>
  </Tab>
  <Tab title="Linux">
...
<Steps>
  ...
  <Step title="Authenticate">
    ...
    If the [Hoop Tunnel](/clients/hsh-tunnel) daemon is installed and reachable, `hsh login` also attempts to authenticate it in the same step. If that step fails, `hsh` prints a warning — run `hsh tunnel login` to authenticate the daemon explicitly, or skip it entirely with `--no-tunnel`.
  </Step>
...
| `hsh tunnel <subcommand>` | Control the [Hoop Tunnel](/clients/hsh-tunnel) daemon (status, connections, up/down, login). |
