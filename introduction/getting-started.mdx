...
Agents and engineers reach real infrastructure with real credentials, and the decision about what they are allowed to do gets made at runtime or not at all. Hoop makes it at runtime.

The **Sidecar** is where you start. It is a proxy that runs next to your resource, decodes the wire protocol, and decides what reaches the database and what comes back — blocking destructive statements, masking sensitive values in responses, and recording every session. Nothing else to deploy and no database to run: one binary, one config file, free.
...
<Card title="Quickstart" icon="rocket" href="/introduction/quickstart">
  Write your first config file and run a Sidecar in front of a local database. You will watch it mask a value on the way out and refuse a destructive statement on the way in.
