...
```sh
curl -sL https://raw.githubusercontent.com/hoophq/hoop/main/deploy/docker-compose/docker-compose.control-plane.yml \
  > docker-compose.yml
```

The file brings up Postgres and the Hoop image started in control plane mode,
plus an optional Sidecar and a database for it to sit in front of. The
subcommand picks the mode: the same image runs the gateway when it is started
with `hoop start gateway`.
