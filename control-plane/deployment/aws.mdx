...
<Warning>
  This stack deploys the **gateway**, not the Control Plane. The template runs
  `hoop start gateway` and takes no parameter to change it, so an install from
  this page reports `application_mode: gateway` and serves the gateway web app.
