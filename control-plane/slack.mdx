...
The review shows the Sidecar, the listener, the statement and one button per group that may approve. **Reject** asks for an optional comment. **More details** opens the review on the Control Plane **Reviews** page, which every signed-in user can open.
...
| Message | Cause | Fix |
|---------|-------|-----|
| `You do not belong to group "…"` | The user is not in that group | Add them to the group in the identity provider, then they open the login link in the message |
| `No Hoop user has the email …` | No hoop user with that email | The reviewer opens the login link in the message, or an admin adds them on the Users page |
