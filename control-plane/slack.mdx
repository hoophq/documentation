...
sequenceDiagram
    ...
    CP->>SL: post the review to the Slack channel
    R->>SL: click Approve
...
- **One channel.** Every review goes to the channel set in **Settings** → **Slack**.
...
<Steps>
  ...
  <Step title="Prepare the channel">
    Choose the channel for reviews. For a **private** channel, invite the bot: type `/invite @Hoop Bot` in the channel.
...
Go to **Settings** → **Slack** → **Configurations**. Paste the **Slack bot token**, the **Slack app token** and the **Slack channel** ID. Click **Save**.

The Control Plane log shows `connected to Slack with Socket Mode`.

Without a channel, a review is filed but nobody is notified in Slack, and the Control Plane logs a warning.
...
- **No message in Slack.** No channel is set, the channel ID is wrong, the bot is not in the private channel, or the log lacks `connected to Slack with Socket Mode`.
...
- One channel for every review, not channels per connection.
