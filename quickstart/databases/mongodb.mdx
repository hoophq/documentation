...
The one-off execution uses the `mongo` utility as a client to execute scripts.
...
The command below creates a connection and uses `mongosh` as the default client for one off executions:

```sh
hoop admin create conn mongodemo -a <myagent> --type database/mongodb \
    -e CONNECTION_STRING=mongodb://user:passwd@127.0.0.1:27017 \
    -- \
    mongosh --quiet '$CONNECTION_STRING'
```
