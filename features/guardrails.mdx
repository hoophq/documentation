...
<Note>
`defer` is the one non-empty `action` a guardrail rule takes. `action: warn` and `action: require_review` are **refused at startup**: both exist only as per-risk-level actions on the [analyzer](/setup/configuration/hoop-sidecar/risk-analysis) block. Human review works only through the analyzer, where `approval_rule` names who may release a held statement. See [Agentic Access](/features/agentic-access#the-tools).
</Note>
