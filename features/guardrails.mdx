...
The AI analyzer is not in this table. It is a per-listener component, configured in its own `analyzer` block beside `guardrails`, and it maps the risk a language model reports onto `block`, `warn`, `allow` or `defer`: the [Agentic Access](/features/agentic-access) path. The deprecated rule spelling `type: ai_analysis` still loads and warns, naming the block as its replacement. See [Risk Analysis](/setup/configuration/hoop-sidecar/risk-analysis).
...
`defer` with no `opa.url` configured loads, and `--validate` prints a note naming the lane. The deferred match then denies instead of reporting a finding, because a finding nobody reads would forward every statement while looking like enforcement.

<Note>
`action: warn` and `require_review` are **refused at startup**. `warn` exists only as a per-risk-level action on the analyzer, which is not a guardrail rule. Human review needs a review backend the current build does not ship — see [Agentic Access](/features/agentic-access#the-tools). For everything else, `defer` is how a rule stops short of deciding.
</Note>
...
<Steps>
  <Step title="Validate the config">
    Nothing binds and nothing is denied. Startup refusals — a bad regex, a rule naming an entity outside `pii.entities` — surface here rather than in front of users.
