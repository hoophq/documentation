...
<Note>
`defer` is the one non-empty `action` a guardrail rule takes. `action: warn` and `require_review` are **refused at startup**: `warn` exists only as a per-risk-level action on the [analyzer](/setup/configuration/hoop-sidecar/risk-analysis), and human review needs a review backend the current build does not ship — see [Agentic Access](/features/agentic-access#the-tools).
</Note>
