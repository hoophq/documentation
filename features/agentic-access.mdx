...
<Note>
`require_review` needs a [Control Plane](/core-concepts/control-plane), which stores the reviews, and an `approval_rule` on the lane naming the access request rule that decides who may approve. It works on `postgres`, `mysql`, `mssql` and `mongodb` lanes, where the client can resend the statement. See [Holding a statement for a person](/setup/configuration/hoop-sidecar/risk-analysis#holding-a-statement-for-a-person) for the retry contract.
</Note>
...
A lane that uses `require_review` always fails closed, whatever `fail_open` says. A failed classification, a spent `max_calls` or an unreachable Control Plane denies there, because forwarding a statement nobody classified would bypass the human gate.
