...
<Note>
`require_review` needs a [Control Plane](/core-concepts/control-plane), which stores the reviews, and an `approval_rule` on the listener naming the access request rule that decides who may approve. It works on every protocol; an ssh listener holds `exec` only and must drop `shell`. See [Holding a statement for a person](/setup/configuration/hoop-sidecar/risk-analysis#holding-a-statement-for-a-person) for the wait, the retry and what each protocol files.
</Note>
