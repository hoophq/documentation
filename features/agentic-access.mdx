...
The listener's `analyzer` block maps each risk level the model can report to a tool.
...
| `defer` | Forward, and hand the decision to [OPA](/setup/configuration/hoop-sidecar/policy-rules#deferring-to-opa), which reads the risk level alongside the actor, the hour and the table. |
| `require_review` | Hold the statement on its connection for up to 30 minutes while a person reviews it. An approval runs it; a timeout denies, and an identical retry after approval forwards once. |

A risk level you do not name defaults to `allow`, so you opt into blocking a tier by writing it down.

<Note>
`require_review` needs a [Control Plane](/core-concepts/control-plane), which stores the reviews, and an `approval_rule` on the listener naming the access request rule that decides who may approve. It works on every protocol, http included; an ssh listener holds `exec` only and must drop `shell`. See [What a hold files](/setup/configuration/hoop-sidecar/risk-analysis#what-a-hold-files) for what the reviewer reads on each protocol.

The wait ends before 30 minutes when the client hangs up, when `idle_timeout_sec` is shorter, or when the caller's own deadline expires. In practice the caller's deadline is the limit: an http client behind Envoy gives up after the 15 s default route timeout. See [Holding a statement for a person](/setup/configuration/hoop-sidecar/risk-analysis#holding-a-statement-for-a-person) for the retry.
</Note>
...
Two pieces: a top-level `analyzer` section that says which model to call, plus the defaults every lane inherits, and the listener's own `analyzer` block that says when to call it and what to do with the answer.
...
The `trigger` is the gate in front of the model call: only statements matching one of its lists — the normalized SQL verb (`operations`), a referenced table (`tables`), or an HTTP resource glob (`resources`) — are classified; everything else forwards for free. Here only `UPDATE` and `DELETE` statements reach the model, and the `high`/`medium`/`low` map decides what happens with the risk that comes back. Omit the trigger and every statement on the lane is classified; see [cost controls](#cost-controls) for what bounds that.
...
The HTTP codec captures nothing by default, so an analyzer block on an HTTP listener without `capture_body` is refused at startup:
...
<Steps>
  <Step title="trigger narrows what is classified">
    Only statements naming these operations, tables or resources are sent. Everything else allows for free. **Omit the trigger and every statement on the lane is classified** — declaring the analyzer is the opt-in, the cache and `max_calls` bound the bill, and `--validate` prints a note naming the per-statement cost. Under `opa.gate: true`, an omitted trigger means Rego decides what is worth classifying, and its silence means skip.
  </Step>
  ...
  <Step title="max_calls is a backstop">
    A budget per lane. `max_calls` on the listener's block bounds that lane's spend; the top-level value is the default every lane inherits. The budget keys on the listener name and survives hot reloads. Past it, statements fall through to the local rules, the same outcome as a listener with no analyzer.
  </Step>
</Steps>
...
A listener that uses `require_review` always fails closed, whatever `fail_open` says. A failed classification, a spent `max_calls` or an unreachable Control Plane denies there, because forwarding a statement nobody classified would bypass the human gate.
...
<CardGroup cols={2}>
  <Card title="AI Analyzer Reference" icon="brain" href="/setup/configuration/hoop-sidecar/risk-analysis">
