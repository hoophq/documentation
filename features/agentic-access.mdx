...
| `require_review` | Deny the first attempt and file it for a person to approve. An identical retry after approval forwards once. |

A risk level you do not name defaults to `allow`, so you opt into blocking a tier by writing it down.

<Note>
`require_review` needs a [Control Plane](/core-concepts/control-plane), which stores the reviews, and an `approval_rule` on the listener naming the access request rule that decides who may approve. It works on `postgres`, `mysql`, `mssql` and `mongodb` listeners, where the client can resend the statement. See [Holding a statement for a person](/setup/configuration/hoop-sidecar/risk-analysis#holding-a-statement-for-a-person) for the retry contract.
</Note>
