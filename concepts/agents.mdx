...
<Warning>
Creating an agent through `hoop admin` doesn't generate a usable token for `hoop run`. Obtain that token by creating a new connection in the web app instead.
</Warning>

## Embedded Mode

Embedded mode runs the agent alongside a running application, sharing its runtime context. Use this mode when you want to:
...
Obtain the key from the web app when creating a connection, then run:

```bash
hoop run -- YOUR_COMMAND --YOUR_FLAGS
```

To run the agent alongside your main application, use `--command` to specify the resource and pass your main process after the `--` delimiter:
...
<Note>
stdout and stderr are linked to your main application and the agent.
</Note>
...
<Warning>
Revoking the organization key will disconnect all agents using it.
</Warning>
