...
| `ALCATRAZ_NER_MODEL_PATH` | Models directory for the Alcatraz NER model, which serves the `PERSON`, `LOCATION` and `NRP` entity types. Set for you by the `-alcatraz` image tags; set it yourself when you mount the model into another image | — |

Agent images differ in what they can run — bundled database clients, RDP, the
Alcatraz NER model — see [Container
Images](/setup/deployment/container-images).
