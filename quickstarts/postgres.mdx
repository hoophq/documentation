...
| Audit | <Icon icon="check" /> * partial | <Icon icon="check" /> | The gateway store and audit the queries being issued by the client |
...
<Info>
**partial:** the connection is only audited when using the "simple query" protocol.
When a client is using the "extended query" protocol for example, the connection is **not audited**.

We plan to add audit support for "extended query" protocol in the near future.
</Info>
