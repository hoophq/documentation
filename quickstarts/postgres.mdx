...
| Audit | <Icon icon="check" /> <Icon icon="asterisk" /> | <Icon icon="check" /> | The gateway store and audit the queries being issued by the client |
...
<Info>
  <Icon icon="asterisk" /> **Partial Support**

  The connection is only audited when using the "simple query" protocol.
  "extended query" protocol queries aren't recorded. You can instruct users to use only "simple query" in their client's settings.

  If you need to block use of "extended query" or want to know the ETA for recording, please [contact us](https://help.hoop.dev/).
</Info>
