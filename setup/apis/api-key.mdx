...
- The `<random-string>` is a secure key string that is going to be used to authenticate your requests
...
<Warning>
    This method of accessing the API uses a **static key with administrator privileges**.
    The **key NEVER expires** and can only be revoked by redeploying the gateway.
    Make sure to keep this key secure and in a safe environment.
</Warning>
