...
| microsoftteams.review.create | Run when an access request is created |
| session.open                 | Run when a session is open and/or access request is created |
...
```sh
hoop admin create svixeventtype session.open --description 'Is sent when a session or access request is created'
