...
<Note>
  If you do not need Presidio's custom recognizers or its Flair models, [Alcatraz](/setup/deployment/alcatraz) provides the same Live Data Masking with no services to deploy — detection runs in-process on the agent.
</Note>
