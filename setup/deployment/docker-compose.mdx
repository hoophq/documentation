...
No configuration required for a local run:
...
Access it at `http://localhost:8009`. For a custom identity provider or remote setup, follow the guide below.
...
<Steps>
  ...
  <Step title="Set the .env file">
    Required for authentication. See all available settings in the [environment variables reference](/setup/configuration/env-vars).
    ...
    <Tabs>
      ...
      <Tab title="Custom Identity Provider">
        Using a custom identity is now handled directly in the Webapp in the **Integrations** > **Authentication** page.
      </Tab>
    </Tabs>
