...
authenticate via SPIFFE, you must leave `HOOP_KEY` unset (or empty). The
agent logs a warning at startup if both are provided so you can spot a
stale `HOOP_KEY` left over from a previous deployment.
</Note>
...
|---|---|---|
| `spiffe.enabled: true` | (toggle) | Renders the spiffe-helper sidecar and SVID volume. Leave `config.HOOP_KEY` unset — HOOP_KEY takes precedence over SPIFFE on the agent, so a stale `HOOP_KEY` value silently disables SPIFFE (the agent logs a warning at startup). |
