...
| `hoophq/hoopagent:<version>-minimal-alcatraz` | As `-minimal`, and masking rules ask for `PERSON`, `LOCATION` or `NRP` |
| `hoophq/hoopagent:<version>-distroless-alcatraz` | As `-distroless`, and masking rules ask for `PERSON`, `LOCATION` or `NRP` |
...
| `hoophq/hoopagent:<version>-minimal-alcatraz` | `-minimal` | The same image plus the Alcatraz NER model |
| `hoophq/hoopagent:<version>-distroless-alcatraz` | `-distroless` | The same image plus the Alcatraz NER model |
...
Data masking with `DLP_PROVIDER=alcatraz` detects most entity types with
patterns, in-process, needing nothing on disk. Three of them — `PERSON`,
`LOCATION` and `NRP` — come from a statistical model instead, and the agent
never downloads it at runtime: the files have to be in the image, or mounted
into it. That is the whole difference between a flavour and its `-alcatraz`
sibling.

Each `-alcatraz` tag is its base flavour plus ~250MB of model weights and one
environment variable, `ALCATRAZ_NER_MODEL_PATH=/opt/alcatraz/models`, which the
image sets for you. The binary, the user, the command and the invocation
contract are identical, so a deployment moves between `-minimal` and
`-minimal-alcatraz` by editing the tag and nothing else.

<Warning>
An agent with no model on disk **refuses** the sessions that ask for those three
entity types, rather than returning them unmasked. Every other entity type keeps
working. If you enable masking rules with `PERSON`, `LOCATION` or `NRP`, deploy
an `-alcatraz` tag — or mount the model into a plain flavour and point
`ALCATRAZ_NER_MODEL_PATH` at it.
</Warning>

The model is loaded lazily, on the first session that needs it, and then costs
about 260MB of resident memory that is not released until the pod restarts.
Size the agent accordingly, and see
[Live Data Masking](/setup/configuration/live-data-masking/get-started#alcatraz)
for the gateway side of the setup.
...
|---|---|---|---|
| `hoophq/hoopagent` (every flavour, `-alcatraz` included) | none | `hoop start agent` | Set `command` — `args` alone would replace the whole command |
