...
<Note>
  On Kubernetes deployments, these variables are set in the `config` section of the Helm chart, or
  loaded from a Secret that already exists in the namespace. See
  [using an existing secret](/setup/deployment/kubernetes#using-an-existing-secret).
</Note>
...
<Tabs>
  ...
  <Tab title="Oauth2/OIDC Authentication">
    ...
    | Environment       | Description   |
    | ----------------- | ------------- |
    | IDP_ISSUER (DEPRECATED)        | Identity Provider Issuer URL (Oauth2). Passing `_userinfo=1` as query string, it forces to authenticate using the [userinfo endpoint](https://openid.net/specs/openid-connect-core-1_0.html#UserInfo).  |
...
| ----------------------------------- | ----------------------- |------------- |
| ADMIN_USERNAME (DEPRECATED)         | admin                   | Changes the name of the group to act as the `admin` role |
| AUDITOR_USERNAME (DEPRECATED)       | auditor                 | Changes the name of the group to act as the `auditor` role |
| API_KEY (DEPRECATED)                |                         | When this environment is set, it enables authentication with full administrative privileges. The key must follow this format: `{org-id}\|{random-string}` |
...
<Warning>
  `ADMIN_USERNAME`, `AUDITOR_USERNAME` and `API_KEY` are deprecated and will be removed in upcoming releases.
  These roles and credentials can now be managed directly via the Webapp or the API.
</Warning>