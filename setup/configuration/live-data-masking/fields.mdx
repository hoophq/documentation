...
Alcatraz ships 45 built-in recognizers covering 12 countries. 25 of them
validate a checksum before reporting a match: Luhn for credit cards, ISO 7064
mod-97 for IBANs, Verhoeff for Aadhaar, mod-11 for the Brazilian documents (CPF,
CNPJ, CNH, PIS) and others. A number with the right shape but the wrong check
digit is dropped rather than masked. It also recognises secrets such as AWS
access keys, JWTs and private keys.

The full set, with ✓ marking the checksum- or format-validated types:
...
| Spain | `ES_NIF`✓, `ES_NIE`✓ |
| Singapore | `SG_FIN`✓ (offered as `SG_NRIC_FIN` in the UI), `SG_UEN` |
| Brazil | `BR_CPF`✓, `BR_CNPJ`✓, `BR_RG`, `BR_CNH`✓, `BR_PIS`✓ |
| Other | `PL_PESEL`✓, `KR_RRN`✓, `FI_PERSONAL_IDENTITY_CODE`✓, `TH_TNIN`✓ |

Every one of these detects a language-independent structured identifier. An IBAN
or a Thai national ID looks the same in any surrounding text, so the whole set is
active regardless of the content's language. The three statistical types below
are English-language.

On top of the built-ins, Alcatraz supports the same **custom recognizers** as
Presidio: regular expressions, compiled as RE2 and therefore without lookaround
or backreferences, and deny lists, matched on word boundaries.

The authoritative list is in the [alcatraz
repository](https://github.com/hoophq/alcatraz).

### Entity types that need the NER model

| Entity type | Detects |
| --- | --- |
| `PERSON` | People's names in free text |
| `LOCATION` | Places, addresses and geographic references in free text |
| `NRP` | Nationality, religious or political group |

These three are statistical, not pattern-based: they come from an ONNX model
that runs in-process on the agent. The agent needs that model on disk, which
means either an `-alcatraz` image tag or a mounted model directory. See [Get
Started](/setup/configuration/live-data-masking/get-started#person-location-and-nrp-need-a-model-on-the-agent).
A rule that selects one of them is refused by an agent without the model; every
other entity type above keeps working.
