...
This service supports two providers for data classification and PII detection: **Alcatraz**, which runs in-process on the agent and needs no extra services, and **Microsoft Presidio**, which runs as a set of services you deploy.
...
Alcatraz needs no services, no credentials and no service URLs — detection is compiled into the agent binary. Set one environment variable in hoop.dev's Gateway:
...
The `PERSON`, `LOCATION` and `NRP` entity types need a small NER model on the agent's disk. The `hoophq/hoopagent` `-alcatraz` image tags ship it.

<Card title="Alcatraz deployment guide" icon="server" href="/setup/deployment/alcatraz">
  Agent images, the NER model, sizing and troubleshooting.
</Card>
