...
description: "Configure time-limited access controls that require approval before a user can connect"
---

**Just-in-Time Access Requests** gate connection access behind a timed approval. For a conceptual overview, see [JIT Access Requests](/learn/features/access-requests/jit).
...
Configure access requests when creating a connection:
...
Users request time-based access with the `--duration` flag:

```bash
hoop connect prod-postgres --duration 2h
```

The CLI waits for approval:
...
Admin users auto-approve their own requests. This ensures admins always have access.
...
These environment variables control JIT Access Requests behavior on the gateway:

| Variable | Description | Default |
|----------|-------------|---------|
| `REVIEW_TIMEOUT_SEC` | Seconds to wait for approval before timing out | `3600` (1 hour) |
