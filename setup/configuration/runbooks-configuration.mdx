---
title: "Runbooks"
description: "Configure Runbook Git repository integration and template engine reference."
---

Configure Runbooks with your Git server using public repos, HTTP credentials, or SSH keys.

<Note>
  This guide covers command-line configuration. The Web App currently supports SSH private keys only, with limited options.
</Note>

First, install the Hoop CLI and log in to your gateway:
...
Basic authentication clones the repository via HTTP using a username and password.
...
SSH authentication clones the repository using a private key.
...
Runbook hooks integrate with any internal workflow, using Hoop to trigger scripts on session events. Place specific Python files in your repository to execute them when events occur.
...
To enable this feature, set `GIT_HOOK_CONFIG_TTL`. This parameter activates runbook hooks and sets the cache time-to-live (in seconds) for the configuration.
...
Runbooks use [Go's text/template](https://pkg.go.dev/text/template). A runbook is a template executed against a connection, with placeholders rendered from inputs provided by an HTTP client.
...
Input specifications are derived from the runbook template. The template below:
...
Fields define how clients create inputs for a runbook. Declare them as function templates in a placeholder using the pipe character `|`. Example:
...
Templates are subject to [code injection](https://en.wikipedia.org/wiki/Code_injection) depending on the runtime. Follow these tips to mitigate the risk:
...
- Use the `asenv` function to pass inputs as environment variables if your runtime supports it. This avoids bash injection via shell control operators:
