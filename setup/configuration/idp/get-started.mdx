---
title: "Overview"
description: "Connect hoop.dev to your Identity Provider using OIDC"
---

Hoop.dev supports a variety of Identity Providers for user authentication. The guides below cover providers we have validated, but any OIDC-compliant IDP should work using the same integration method.
...
- The user profile is derived from the `email` and `name` claims in the `id_token`.

On first authentication, the platform performs an automatic signup that persists profile claims alongside the unique identifier.
...
<Tip>
Groups can be managed manually in the Webapp or propagated by the identity provider via the ID Token. When using IdP propagation, groups sync each time a user logs in.
</Tip>
