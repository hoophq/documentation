...
Deploy Hoop Gateway behind a reverse proxy for load balancing and TLS termination. Both ports must be configured correctly for all functionality to work.
...
<AccordionGroup>
    ...
    <Accordion title="gRPC Service Ingress">
        ...
        ```
        <Info>
            The gRPC ingress configuration has two important directives to proxy the connections:
...
<Tip>
  Do not define timeout or retry policies for gRPC connections (port 8010). Long-lived connections will fail if timeouts are set.
</Tip>
