...
description: "Create and manage pattern-based rules to block dangerous queries before they execute"
---
...
Each guardrail can be assigned to multiple connections. A single connection can also have multiple guardrails.
...
When multiple guardrails apply to a connection, they evaluate in priority order:

1. **Priority 1** (highest) evaluated first
2. First matching rule determines the action
3. If no rules match, the query is allowed
...
These environment variables control guardrails behavior on the gateway:
