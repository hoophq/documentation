...
description: "Expand connection environment variables dynamically from AWS Secrets Manager, HashiCorp Vault, or environment JSON."
---

## AWS Secrets Manager Provider

This provider expands connection environment variables from an AWS key-value secret.
...
The value is replaced when a user opens a session with the agent.
...
This provider expands environment variables from the [Vault Key Value Secrets Engine](https://developer.hashicorp.com/vault/docs/secrets/kv). Supports versions 1 and 2.
...
The **approle** auth method lets machines and apps authenticate with Vault-defined roles. It targets automated workflows, not human operators.

The agent requests `POST /auth/approle/login` and obtains a token to access secrets in the Vault Key Value store.

Configure these environment variables when deploying the agent:
...
This provider exposes environment variables from a JSON environment variable set on the agent. Use it to maintain compatibility with older runops agents.
