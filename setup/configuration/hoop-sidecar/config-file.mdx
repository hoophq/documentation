...
| `http_header` | Every header in `headers` matches one of its patterns and every header in `headers_not` matches none (or is absent); requests only | `headers`, `headers_not`, `resources`, `methods` |
...
| a `table`, `http_resource`, `http_status`, `http_header` or `grpc_status` rule | An SSH statement has nothing for them to read. Match a path with `pattern_match` scoped to the `sftp_*` operations instead. |
