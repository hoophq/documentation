...
The Sidecar skips OPA for that exchange's response statements and asks again on the next request. It reads the field on request statements only, on either phase of a two-phase lane, and one answer covers both OPA calls. An exchange is one request and the responses that answer it; a `client` statement that follows a `server` one starts the next. A client-streaming RPC repeats the opt-out on each request message at no extra cost, since request statements reach OPA in full regardless. Absent or `true` changes nothing.
