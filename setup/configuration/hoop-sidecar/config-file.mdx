...
| `downstream_tls` | Terminates the client's TLS at the Sidecar. Only valid on a `postgres` lane (pgwire negotiates in-band) or a `grpc` lane (it is its own HTTP/2 endpoint). Refused on `ssh`, which negotiates its own transport. See [Kerberos and SQL Server](/setup/configuration/hoop-sidecar/kerberos#terminating-the-clients-tls). |
...
MySQL `SSLRequest`, then completes authentication inside TLS. It also bridges
the RSA exchange used by `caching_sha2_password` and `sha256_password`.
