...
<Note>
New to hoop-inspect? Start with [Get Started](/setup/configuration/hoop-sidecar/get-started), which builds a working config one step at a time.
</Note>
...
| `grpc` | Descriptor sets and capture for a gRPC lane. Only valid on a `grpc` lane. See [gRPC](/setup/configuration/hoop-sidecar/protocols/grpc#the-grpc-block). |
...
Keep the middle leg on loopback or a unix socket. It carries decrypted traffic, and a socket is the tighter boundary because no port exists to reach. See [Terminating client TLS](/setup/configuration/hoop-sidecar/get-started#terminating-client-tls) for the Envoy config.
...
<CardGroup cols={2}>
  ...
  <Card title="Get Started" icon="rocket" href="/setup/configuration/hoop-sidecar/get-started">
