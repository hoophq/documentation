...
| `downstream_tls` | Terminates the client's TLS at the Sidecar. Only valid on a `postgres` lane, because pgwire negotiates in-band. See [Kerberos and SQL Server](/setup/configuration/hoop-sidecar/kerberos#terminating-the-clients-tls). |
...
| `http` | What the HTTP codec captures. Only valid on an `http` lane. See [Risk Analysis](/setup/configuration/hoop-sidecar/risk-analysis#the-http-lane-needs-capture-body). |
