...
**An analyzer block that would classify nothing:**

```
Error: invalid config:
  - appdb: analyzer block has no trigger, so it would classify nothing; name
    operations, tables or resources, or turn on opa.gate and let the policy
    decide
```
