...
| `protocol` | `postgres`, `mysql`, `mssql`, `mongodb`, `grpc`, `http` or `ssh`. Selects the codec; see [Protocols](/setup/configuration/hoop-sidecar/protocols/postgres) for what each one decodes. `ssh` is the exception: it has no codec, because it **terminates** the protocol instead of relaying it. |
...
for client modes and a verification query.
