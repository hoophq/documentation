...
| `protocol` | `postgres`, `mysql`, `mssql`, `mongodb`, `grpc` or `http`. Selects the codec; see [Protocols](/setup/configuration/hoop-sidecar/protocols/postgres) for what each one decodes. |
...
| `downstream_tls` | Terminates the client's TLS at the Sidecar. Only valid on a `postgres` lane (pgwire negotiates in-band) or a `grpc` lane (it is its own HTTP/2 endpoint). See [Kerberos and SQL Server](/setup/configuration/hoop-sidecar/kerberos#terminating-the-clients-tls). |
