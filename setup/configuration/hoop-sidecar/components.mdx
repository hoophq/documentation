...
The guardrails engine and OPA fail closed. An unreachable OPA, a 500, or an undefined decision denies. One exception: an undefined **gate** decision allows and requests nothing, because a gate is an optimization over a policy someone already wrote. The analyzer inverts the default, because it depends on a third-party API rather than a service you run. A lane where the analyzer holds statements for review keeps failing closed. See [Risk Analysis](/setup/configuration/hoop-sidecar/risk-analysis#why-this-one-fails-open).
