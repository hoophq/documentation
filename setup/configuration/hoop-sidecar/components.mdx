...
|---|---|
| `http`, `grpc`, `spanner` | the header named by `identity_header`, set by the authenticating proxy in front. An `http` lane reads it from the first request on the connection, before the session exists; a grpc lane reads it per RPC and falls back to the mTLS peer certificate |
