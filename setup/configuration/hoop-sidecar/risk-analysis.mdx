...
listeners:
  ...
  - name: api
    ...
    http:                          # what the model sees, see below
      capture_body: true
      max_body_bytes: 8192
...
| `require_review` | File the statement with the Control Plane and wait up to 30 minutes for a person to approve it. An approval runs it; a timeout denies, and an identical retry after approval forwards once. See [Holding a statement for a person](#holding-a-statement-for-a-person). |
...
Every protocol can hold. What the reviewer reads differs per protocol; see [What a hold files](#what-a-hold-files).
...
The wait and the poll interval are constants, with no config field. A Control Plane older than the Sidecar has no poll route, so the Sidecar denies after the first poll.

**The wait often ends before 30 minutes.** Each of these stops it before an approval is claimed, so the approval stays for the retry:

| Cause | Note |
|---|---|
| The client hangs up or the upstream closes | The audit record carries the cause. Ctrl-C in `psql` or `mysql` does **not** end the wait: both send the cancel on a separate connection. Close the client instead. |
| `idle_timeout_sec` is shorter than 30 minutes | A held connection is idle. `--validate` prints a note. |
| The caller's own deadline expires | In practice this sets the limit on http, grpc and spanner. An http client behind Envoy gives up after the 15 s default route timeout. |

**A timed-out statement is released by a retry.** The developer asks an approver, then runs the same statement again:

<Steps>
  <Step title="The retry collects the approval">
...
**postgres, mysql, mssql, mongodb and clickhouse** file the statement as the codec reads it.

**http** files the method, the target and the body: `POST /transfers?dry_run=false`, a blank line, then the body. The analyzer already needs `capture_body: true`. Five consequences:
...
**ssh** holds `exec` only, because the analyzer classifies `exec_line` alone. A shell sends no statements, so a user could type in a shell what `exec` would hold. A holding ssh listener must therefore drop `shell` from [`capabilities_allowed`](/setup/configuration/hoop-sidecar/protocols/ssh/configuration#capabilities-allowed). An absent key admits `shell`, so write the list out:
...
<Tabs>
  <Tab title="Google Vertex">
    ...
    analyzer:
      ...
      extra:
        ...
        # publisher: anthropic  # the default; see the table below for google and openapi
      # credentials_file omitted -> Application Default Credentials
    ```
    ...
    You choose the model family with `publisher`, and the relay calls that family's Vertex API:

    | `publisher` | Models | Vertex API | `model` format |
    |---|---|---|---|
    | `anthropic` (default) | Claude | `rawPredict` | `claude-sonnet-4-5@20250929` |
    | `google` | Gemini | `generateContent` | `gemini-2.5-flash` |
    | `openapi` | Model Garden open models with an API endpoint, such as Llama, DeepSeek, Qwen and gpt-oss | OpenAI-compatible Chat Completions | `<publisher>/<model>`, e.g. `meta/llama-4-maverick-17b-128e-instruct-maas` |

    One bearer token covers all three. The relay keeps no model list, so any model name Vertex accepts works, and it refuses an unknown `publisher` at startup.

    For Gemini, set `publisher: google` and a Gemini model:
  ...
  <Tab title="Gemini (API key)">
    ...
    analyzer:
      ...
      # extra: {api: vertex}   # Vertex AI express mode with a Google Cloud API key; the default is the Gemini Developer API
    ```

    `gemini` takes a Google API key, in the `x-goog-api-key` header. Omit `extra.api` for the Gemini Developer API on `generativelanguage.googleapis.com`, billed to a Google account. Set `api: vertex` for Vertex AI in express mode on `aiplatform.googleapis.com` with a Google Cloud API key, billed to the key's project.

    Express mode is global. The URL names no location and Google routes the call, so it offers no region selection and no VPC Service Controls. A workload that must stay in one region uses the Google Vertex tab with `publisher: google`, where `region` is part of the URL.
...
<Tabs>
  <Tab title="Workload Identity (best)">
    ...
    publishers: Claude, Gemini and the open models.
...
| `require_review` on an ssh listener that admits `shell` | Refused: a shell sends no statements, so what is typed in it goes around the hold. Drop `shell` from `capabilities_allowed` |
...
| An analyzer on a gRPC lane without `grpc.capture_payload` | Refused: every RPC would be skipped |
...
| A Vertex `publisher` other than `anthropic`, `google` or `openapi` | Refused, listing the three |
| `publisher: openapi` with a model lacking its `<publisher>/` prefix | Refused: the shared endpoint cannot route it |
