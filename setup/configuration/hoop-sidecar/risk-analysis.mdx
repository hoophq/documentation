...
<Steps>
  ...
  <Step title="max_calls is a backstop">
    A process-lifetime budget for the lane. Past it, statements fall through to the guardrails and OPA, the same outcome as a lane with no analyzer. A lane that [holds statements for review](#holding-a-statement-for-a-person) denies instead: a statement with no risk level cannot be forwarded past a human gate. The budget keys on the listener name, so a hot reload that edits the block continues the running count rather than re-arming it.
  </Step>
</Steps>
...
`require_review` refuses a statement until a person approves it. `approval_rule` names the access request rule in the [Control Plane](/core-concepts/control-plane) that decides who may approve: the reviewer groups, the approval count and the force-approval list. The lane holds only the rule's name, and the Control Plane authorizes each review against the configuration it stored for that Sidecar.
...
`approval_rule` is per lane, so the people who may release a statement against the payments database are not, by accident, the people who may release one against a reporting replica. Editing the block hot-reloads with the lane's rules.
...
**The approval is exact; the classification is not.** The [cache](#the-three-cost-controls) keys on the statement shape with literals stripped, so `WHERE tenant = 'test'` and `WHERE tenant = 'prod'` share one verdict. A shape rated high holds every statement of that shape, each filing its own review. A shape rated low forwards without a hold, even when a later literal makes it the dangerous one. Set `cache: {size: 0}` on a lane where every statement must be judged on its own, and pay one model call per statement.

**A holding lane fails closed.** `fail_open` answers for a model vendor's outage, not for a human gate, so it is not consulted on a lane where any risk level asks for `require_review`. Each of these denies there:
...
A statement the trigger does not match still forwards, as on any lane.
...
A lane that holds statements for review is the exception. There `fail_open` is not consulted, and a failed classification, a spent `max_calls` or an unreachable Control Plane denies. See [Holding a statement for a person](#holding-a-statement-for-a-person).
...
<AccordionGroup>
  <Accordion title="Report the verdict by calling exactly one risk tool">
    The risk level *is* which tool the model chose, which makes it a three-value enum instead of a parsing problem. Without this instruction the model answers in prose, nothing maps to a level, and every statement fails classification. Under `fail_open: true` that allows everything, and on a lane that holds statements for review it denies everything.
  </Accordion>
...
| `require_review` on a lane that is not `postgres`, `mysql`, `mssql` or `mongodb` | Refused: a hold needs a client that resends the statement. An HTTP or gRPC caller reads a refusal, and an SSH session is already closed |
| `require_review` on a Sidecar with no Control Plane | Refused: there is nowhere to file the review |
| `require_review` on a deprecated `ai_analysis` rule | Refused: the rule carries no `approval_rule`. Move the lane to a listener `analyzer` block |
...
- **A model can refuse to classify.** The reply carries `stop_reason: refusal` and no verdict, and under `fail_open: true` the statement is allowed. A lane that holds statements for review denies it instead. The Sidecar records `model refused to classify this statement`. Watch for these in the audit trail while the lane is still on staging: a model that refuses is the wrong model for this job.
