...
Some requests carry their intent in a header. kubectl asks for a Secret's table view with `Accept: application/json;as=Table;...` and for its contents with `Accept: application/json`, on the same GET. This rule matches the second and lets the first through.

`headers` maps a header name to the values that match it. Every named header must match; any listed value will do; an empty list means the header is present with any value. Values compare case-insensitively. A bare `*` matches any run of characters (`kubectl delete*`), and `\*` is a literal star (`\*/\*` for `Accept: */*`). `methods` and `resources` scope the rule as they scope `http_resource`.

The lane's codec fills a statement's headers from `http.headers` and nothing else. A rule naming a header the lane does not capture is refused at startup with the name to add; it would load and match nothing.
