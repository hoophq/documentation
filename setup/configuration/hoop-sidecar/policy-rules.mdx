...
A second field, `responses`, works on any phase and on request statements only. `{"allow": true, "responses": false}` tells the Sidecar to stop asking OPA about the response side of that exchange, which on a capturing gRPC lane is the difference between one call and one call per streamed message. See [Responses](/setup/configuration/hoop-sidecar/config-file#responses).
