...
<Tabs>
  ...
  <Tab title="AI analysis, gated">
    ...
    The analyzer block carries no `trigger`, which a plain lane refuses. A gated lane accepts it, because the gate decides what gets classified and the trigger would be a second cost control the Rego author never sees.
...
| an analyzer block with no `trigger`, no gate | `analyzer block has no trigger, so it would classify nothing; name operations, tables or resources, or turn on opa.gate and let the policy decide` |
