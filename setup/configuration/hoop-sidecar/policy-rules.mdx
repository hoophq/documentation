...
| `http_header` | the value of one or more captured headers | HTTP |
...
| Lane | Refuses |
|---|---|
| `ssh` | `table`, `http_resource`, `http_status`, `http_header`, `grpc_status` — SSH has no relations, no request path, no response status, no header and no RPC |
...
Patterns match the NORMALIZED path, so `/users/12345/orders/98765` arrives as `/users/*/orders/*` and one rule replaces a regex per endpoint. A trailing `/**` matches the path itself and anything under it, and a `*` segment before it stays a wildcard: `/api/v1/namespaces/*/secrets/**` covers the collection in every namespace and every secret in it. `methods` narrows the rule; leave it out to cover every method.
...
Some requests carry their intent in a header. kubectl asks for a Secret's table view with `Accept: application/json;as=Table;...` and for its contents with `Accept: application/json`, on the same GET. The rule above names the table view as the one safe shape and refuses every other request on a secret: `-o yaml`, `Accept: application/json;q=1`, Protobuf, and a curl that sends no `Accept` at all.

Two maps, and a rule uses one or both:

- `headers_not` names the safe shapes. The rule matches unless every named header is present with a matching value. Write a rule that protects something this way, so the request you did not think of is denied.
- `headers` names the unsafe shapes. The rule matches when every named header is present with a matching value: `kubectl-command: ["kubectl delete*"]` refuses a delete however kubectl spells the path. A request without the header cannot match this form.

Both map a header name to value patterns. Every named header must satisfy its map; any listed value will do; an empty list means present (`headers`) or absent (`headers_not`). Values compare case-insensitively. A bare `*` matches any run of characters, and `\*` is a literal star. Only a request matches: a response carries the server's headers, and this rule reads the client's intent. `methods` and `resources` scope the rule as they scope `http_resource`.

The lane's codec fills a statement's headers from `http.headers` and nothing else. A rule naming a header the lane does not capture, in either map, is refused at startup with the name to add; it would load and match nothing.
...
<Note>
`type: ai_analysis` under `guardrails.rules` is the **deprecated** spelling. It still loads and enforces, prints a deprecation naming the listener block, and `--validate --strict` exits non-zero. The rule form takes no `action` field (it defers per risk level, `high: defer`), and the block has no `action` field at all. `require_review` works only on the block: the rule form carries no `approval_rule` to name who may release a held statement, so it refuses the action. See [Holding a statement for a person](/setup/configuration/hoop-sidecar/risk-analysis#holding-a-statement-for-a-person).
</Note>
...
A second field, `responses`, works on any phase and on request statements only. `{"allow": true, "responses": false}` tells the Sidecar to stop asking OPA about the response side of that exchange, which on a capturing gRPC lane is the difference between one call and one call per streamed message. Only `grpc` and `spanner` lanes honor it, since they run one gate per RPC; a relay lane ignores it and marks the request `opa.responses_unscoped: connection`. See [Responses](/setup/configuration/hoop-sidecar/config-file#responses).
