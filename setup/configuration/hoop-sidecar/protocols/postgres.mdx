...
| `P` Parse | The extended protocol: the prepared statement every driver and ORM actually uses |

Everything else is skipped by length, so the `COPY` stream of a bulk load is never buffered into memory. A statement split across TCP segments is reassembled before it is classified.

A multi-statement `Q` message is split with PostgreSQL's own lexical rules — dollar quoting, standard-conforming strings, nested comments — and each statement is evaluated on its own. `Operation` is the statement's most consequential **effect**, so a `DELETE` hidden inside a CTE classifies as `delete`; see [the worked example](/setup/configuration/hoop-sidecar/get-started#step-5-watch-it-work).

<Warning>
`CALL` and `EXECUTE` report `unknown`, because their bodies live in the catalog and no parser can say what they touch. A rule that must catch them writes `operations: [call, unknown]`.
</Warning>
...
`postgres` is one of two protocols that may terminate the **client's** TLS at the lane (the other is `grpc`), precisely because nothing generic in front can speak the in-band exchange:
...
On the upstream side two details are deliberate:

- **A refusal is an error, not a downgrade.** A server answering `N` to the `SSLRequest` fails the connection. You asked for an encrypted hop; silently sending credentials in the clear is the failure you were preventing.
- **Channel binding is stripped.** The Sidecar removes `SCRAM-SHA-256-PLUS` from the server's SASL offer, because channel binding ties SCRAM to a single TLS session and a terminating relay has two. Plain `SCRAM-SHA-256` remains, authenticating the same password against the same verifier. No credential or server change needed.
...
Column rules match the names in the result set's `RowDescription`, so they beat detection wherever the protocol names its values.
...
Envoy has no pgwire parser, so the lane is plain `tcp_proxy` and every byte reaches the Sidecar unexamined:
...
To terminate the client's TLS in Envoy instead of at the lane, use the contrib `postgres_proxy` filter with a `starttls` transport socket — the full shape and its caveats are in [Terminating client TLS](/setup/configuration/hoop-sidecar/get-started#terminating-client-tls).
