...
A `postgres` lane decodes the PostgreSQL v3 frontend/backend protocol: every statement a client sends, and every row the server returns. It is the lane the compose stack demonstrates and the one [Running the Sidecar](/install/cli#running-the-sidecar) builds step by step.
...
A `postgres` lane can terminate the **client's** TLS itself because a generic
...
To terminate the client's TLS in Envoy instead of at the lane, use the contrib `postgres_proxy` filter with a `starttls` transport socket. [Terminating client TLS](/install/cli#terminating-client-tls) has the full shape and its caveats.
...
<CardGroup cols={2}>
  <Card title="Running the Sidecar" icon="play" href="/install/cli#running-the-sidecar">
