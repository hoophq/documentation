...
MySQL's `caching_sha2_password` and `sha256_password` plugins need an RSA
exchange on the plaintext client leg. The lane supplies an ephemeral public
key, decrypts the client's response, and sends the recovered password only
inside the verified upstream TLS connection. Other authentication plugin
