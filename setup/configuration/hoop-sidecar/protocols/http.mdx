...
See [Kubernetes API](/setup/configuration/hoop-sidecar/protocols/http/kubernetes) for `columns: [data]` against kube-apiserver's chunked responses, and the [ClickHouse](/setup/configuration/hoop-sidecar/protocols/clickhouse) HTTP interface for TSV, `JSONEachRow` and `FORMAT JSON` results.
