...
The codec rebuilds the response around the masked values. A JSON body is walked value by value, and each string or number reaches the masker under its dotted key path (`data.password`, `items.metadata.name`), so a `columns` rule names a JSON key the way it names a result-set column and masks it whatever it holds. A text body (`text/*`, XML, YAML, form data, or no `Content-Type`) is one cell for entity detection. Binary types and encodings other than gzip pass through untouched.

`Content-Length` is corrected. A chunked body is re-chunked and streams: each complete top-level JSON value, or each chunk of text, goes out as soon as it is whole, so a watch or a log follow is not held to its end. A gzip body is inflated around the masker and compressed again. A body the codec is holding that grows past 16 MiB goes out unmasked, and the next response is masked again.

See [Kubernetes API](/setup/configuration/hoop-sidecar/protocols/http/kubernetes) for `columns: [data]` against kube-apiserver's chunked responses.
...
<CardGroup cols={2}>
  <Card title="Kubernetes API" icon="dharmachakra" href="/setup/configuration/hoop-sidecar/protocols/http/kubernetes">
  ...
  <Card title="Guardrail Rules" icon="shield-halved" href="/setup/configuration/hoop-sidecar/policy-rules">
    `http_resource` globs, `http_status` ranges, `http_header` patterns, and deferring a match to Rego.
  </Card>
