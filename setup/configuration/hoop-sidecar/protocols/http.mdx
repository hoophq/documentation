...
| Field | Meaning |
|---|---|
| `capture_body` | Includes request and response bodies in the statement. Required for `pii`/`pattern_match` rules that must read payloads, and for the lane's analyzer. Startup refuses those without it, because they would load and then fire on nothing. |
...
<CardGroup cols={2}>
  ...
  <Card title="Risk Analysis" icon="brain" href="/setup/configuration/hoop-sidecar/risk-analysis">
    Why the lane's analyzer requires `capture_body`, and what it costs.
  </Card>
</CardGroup>
