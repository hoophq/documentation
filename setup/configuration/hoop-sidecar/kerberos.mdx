...
The Sidecar relays Kerberos-authenticated database sessions and inspects each
statement inside them. It holds no credential, reads no ticket, and contains no
Kerberos code.

A user runs `kinit`, their OS mints a ticket, and the Sidecar carries it to the
...
flowchart LR
    ...
    E -->|plaintext| R["Sidecar<br/>inspects · masks · audits"]
...
<Note>
For the commands, start at [Get Started](/setup/configuration/hoop-sidecar/get-started).
For every config field, see the [Config File reference](/setup/configuration/hoop-sidecar/config-file).
</Note>
...
the Sidecar forward a credential it cannot interpret.

| Packet | Type | What the Sidecar does |
|---|---|---|
| PRELOGIN | `0x12` | forward, TLS handshake wrapped inside it included |
| LOGIN7 | `0x10` | forward; record that the login is integrated |
| SSPI | `0x11` | forward verbatim, carrying the Kerberos exchange |
| encrypted login | raw TLS records | forward, then resume on the byte where plaintext returns |
...
Clients connect with `Encrypt=strict`.

### TDS 7.x encrypts the login and nothing else

A 7.x client wraps its TLS handshake inside `0x12` packets, which Envoy cannot
...
| Client asks | On the wire | The Sidecar |
|---|---|---|
| `ENCRYPT_OFF` (`Encrypt=optional`, go-mssqldb default) | login in TLS, statements clear | inspects |
| `ENCRYPT_ON` (`Encrypt=mandatory`, ODBC 18 and SqlClient 5+ defaults) | whole session in TLS | refuses |
| TDS 8.0 reaching the Sidecar unterminated | TLS from the first byte | refuses |

<Warning>
Both refusals report `rule: stream-unsafe` and name the cause. A session nobody
can inspect used to connect and run with no policy, no masking and no audit
trail, and it now stops instead. Check what your driver sets for `Encrypt`
before you put an existing MSSQL lane behind this.
</Warning>
...
Forward it and the client lands on a socket the Sidecar has no hold on, where the
session continues with no policy, no masking and no audit, leaving no trace
that it stopped being watched. The Sidecar refuses it and ends the connection with a
...
At the moment the Sidecar reads it the name is a claim. It becomes true when the
...
Three compose stacks under `deploy/docker-compose/envoy-stack` run the whole
...
# TDS 7.4, no Envoy: the login is encrypted and the statements are not.
# MSSQL_TAG accepts 2017-latest, 2019-latest and 2022-latest.
hoopinspect/scripts/dev/mssql2019-stack.sh
hoopinspect/scripts/dev/mssql2019-check.sh
```
...
down two paths, through the Sidecar and straight at the database, and reports a
disagreement as a failure. A Kerberos result tells you about the Sidecar only when
