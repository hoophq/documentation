...
description: "Configure group-based access policies to restrict which users can see and connect to specific connections"
---
...
**Access Control** restricts which users can see and access specific connections based on group membership. This page covers detailed configuration options.
...
<Warning>
Enabling Access Control immediately hides connections from users without explicit access. Plan your group assignments before enabling.
</Warning>
...
<Steps>
  ...
  <Step title="Review the Warning">
    Read the activation warning before proceeding. Understand that:
    - Users without group assignments lose access immediately
    - You must configure each connection individually
...
Groups sync automatically when users log in:

1. Configure your IdP to include a `groups` claim in the ID token
2. Groups sync on each login
...
To sync groups automatically from your identity provider:

1. **Configure the IdP to include a groups claim:**
...
3. **Groups sync on login:**
   ...
   When users log in, their groups sync automatically from the IdP.
...
This is not an Access Control issue. Check:
