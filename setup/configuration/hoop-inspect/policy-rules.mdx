---
title: "Policy Rules"
description: "Write a rule, pick deny or defer, and hand the judgement call to Rego"
---
...
[Config File](/setup/configuration/hoop-inspect/config-file#policy-rules) carries the exhaustive schema for every field named below.
...
<Steps>
  ...
  <Step title="The policy sees it in input.findings">
    ...
    The relay fills `input.context` from the session: `principal`, `session_id` and `connection` always, plus `subject`, `email`, `groups`, `peer_addr`, `upstream` and `correlation_id` where the identity carries them.
  </Step>
...
Four complete lane configs. Each one validates with `hoop-inspect -validate -config config.yaml`.
