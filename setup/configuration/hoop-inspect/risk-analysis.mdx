...
A `DELETE` that a `type: operation` rule already refuses never reaches a model. That ordering is not an optimization you can turn off; it is what makes the feature affordable.
...
<Steps>
  ...
  <Step title="max_calls is a backstop">
    A process-lifetime budget. Past it, statements fall through to the local rules and OPA — the same outcome as a lane with no analyzer, which is what a spent budget means.
  </Step>
</Steps>
...
<Warning>
`require_review` is **refused at startup**. Holding a statement for human approval needs a review backend this build does not have, and a config that looks like it holds statements for approval while quietly forwarding them is worse than one that fails.
</Warning>
...
The analyzer classifies `FromClient` statements and ignores responses. By the time a response comes back the write has already run, so a verdict cannot prevent anything — and read-side exposure is [masking](/setup/configuration/hoop-inspect/config-file#masking)'s job, which is cheaper and already running.
...
Failing open is not failing silently. The verdict still carries the error, so it reaches the audit trail and `/stats` counts it. And the local rules and OPA both ran and both allowed — a silent analyzer means "guarded by everything except the optional expensive layer", not "unguarded".
...
HTTP headers never reach the model, even ones a lane allowlists for policy. An allowlist that is safe for a local rule is not automatically safe to hand a third party.

---

## Writing your own prompt

What counts as risky depends on what you are protecting, so the guidance is replaceable at two levels.
...
<AccordionGroup>
  <Accordion title="Report the verdict by calling exactly one risk tool">
    The risk level *is* which tool the model chose. That is what makes it a three-value enum instead of a parsing problem. Without this instruction the model answers in prose, nothing maps to a level, and every statement fails classification — which under `fail_open: true` means allowing everything.
  </Accordion>
...
Both failures are silent — the classifier keeps answering, just worse and leakier — which is exactly why they are not a config surface.
...
A request with no body is **skipped rather than classified**, so a forgotten `capture_body` shows up as an analyzer that never fires rather than as an error. If an HTTP rule seems inert, check this first.
...
<Tabs>
  <Tab title="Google Vertex">
    ...
    Vertex authenticates with a GCP OAuth bearer minted from a service account and refreshed automatically. `--validate` mints one token, so a bad key, a missing `roles/aiplatform.user` binding or a skewed clock fails the config check rather than the first risky statement.

    <Tip>
    Prefer Workload Identity and omit `credentials_file` entirely. On GKE there is then **no credential on disk at all** — the pod's identity is the credential, so there is nothing to leak or rotate.
    </Tip>
...
- **A verdict is a model's opinion, sampled once.** The same statement can classify differently on two runs, and the cache freezes whichever answer came first for its TTL. Keep the risks you *can* describe in an `operation` or `table` rule: free, deterministic, and immune to a vendor outage.
- **Requests only.** A response verdict cannot prevent a write that already ran.
- **No human review.** `require_review` is declared in the schema and refused at startup, so a config written today stays valid when a review backend lands.
