...
Nothing above the transport changes. Policy, masking, audit and `upstream_tls` behave the same way, because the gate reads a `net.Conn` and never asks what kind it is. See [Transport](/setup/configuration/hoop-inspect/config-file#transport) for the full comparison.
...
<Note>
`type: operation` matches the statement's most consequential **effect**, so `WITH d AS (DELETE FROM customers RETURNING *) SELECT count(*) FROM d` counts as a `delete` and the rule above catches it. [Policy Rules](/setup/configuration/hoop-inspect/policy-rules) covers every rule type, including deferring a match to a Rego policy instead of denying it.
</Note>
...
<Tabs>
  ...
  <Tab title="MSSQL lane">
    ...
    Clients connect with `Encrypt=strict`. TDS 7.x wraps its TLS handshake inside `0x12` packets, which Envoy cannot speak, so a 7.x lane needs a terminator that does.
  </Tab>
  <Tab title="HTTP lane">
...
The relay reads plaintext. Something in front has to decrypt whatever the client encrypted before the gate sees a statement; the relay terminates **no** downstream TLS.
...
| relay → database | the relay, via [`upstream_tls`](/setup/configuration/hoop-inspect/config-file#upstream-tls) |
...
<Note>
SQL Server runs in its own stack under `deploy/docker-compose/envoy-stack/mssql/`, driven by `hoopinspect/scripts/dev/mssql-stack.sh`: Envoy terminating TDS 8.0, a Samba AD DC and a client holding a ticket. Microsoft publishes no arm64 image, so first boot on Apple Silicon takes minutes. See [Kerberos and SQL Server](/setup/configuration/hoop-inspect/kerberos#local-testing).
</Note>
