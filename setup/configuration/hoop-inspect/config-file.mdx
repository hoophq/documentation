...
<Note>
**Prefer a socket.** A TCP listener on 15432 is reachable by anything that can route to the host, and a NetworkPolicy narrows that without removing it. A unix socket opens no port at all, so reachability becomes a filesystem question, which is the point of a sidecar sharing a namespace with one workload. The cost is coordination: both processes need the same directory and their uids have to agree. That is cheap in a pod spec and awkward on a laptop.
</Note>
