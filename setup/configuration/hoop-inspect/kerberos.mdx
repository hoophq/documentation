...
speak, so no terminator sits in front of that lane. The relay takes the
...
The relay walks that encrypted region by TLS record framing, resumes on the
...
| Client asks | On the wire | The relay |
|---|---|---|
...
| TDS 8.0 reaching the relay unterminated | TLS from the first byte | refuses |
