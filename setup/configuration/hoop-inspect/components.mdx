...
flowchart TB
    ...
    subgraph si["hoop-inspect process"]
      LH["lane httpbin<br/>http.sock · http codec"]
      LP["lane appdb<br/>pg.sock · postgres codec"]
    ...
    L8443 -- "unix socket" --> LH --> H
    L5432 -- "unix socket" --> LP -- "TLS (pgwire StartTLS)" --> D
```
...
```
envoy :8443 ──cluster hoop_inspect_http──> http.sock  lane "httpbin"   http
envoy :5432 ──cluster hoop_inspect_pg────> pg.sock    lane "appdb"     postgres
                                                      └─ one process
```

Each lane binds a unix socket here. Swap either for a `host:port` and nothing above the transport changes, because the gate reads a `net.Conn` and never asks what kind it is.
...
### The recommended shape: Envoy over a unix socket

A TCP listener on 15432 is reachable by anything that can route to the pod. A NetworkPolicy narrows that; it does not remove it. With a socket there is no port, so reachability stops being a network question and becomes a filesystem one, which is the argument for a sidecar sharing a namespace with one workload.
...
### When a socket does not fit

Sockets need both processes to mount one directory and agree on uids. That is cheap in a pod spec and awkward where the relay and its peer sit on different hosts. Drop `network` from the lane, give `listen` a `host:port`, and use a `STRICT_DNS` cluster. Bind loopback where the two share a network namespace: a `0.0.0.0` bind is reachable by anything that can route to the host.
