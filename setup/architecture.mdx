...
Hoop is a secure access gateway that sits between your users and your infrastructure. It proxies connections through a lightweight agent running inside your network. No inbound firewall rules required, no VPN needed, and nothing in your infrastructure is exposed to the internet.
...
The agent runs inside your network, close to the resources you want to expose. It establishes an outbound gRPC connection to the gateway, so your infrastructure never needs to accept inbound connections.
...
Optional components (data masking via Microsoft Presidio and protocol proxies) can be enabled independently.
