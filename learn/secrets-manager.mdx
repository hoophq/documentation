...
It requires the environment variables exported in the machine running the agent. The implemetation follows the specification of the [Vault Cli](https://developer.hashicorp.com/vault/docs/commands#environment-variables) and it's limited to the configuration below:
...
```sh
hoop admin create conn pgprod --type database/postgres -a <agent-name> \
    -e HOST=_vaultkv1:/kv/pgprod:DBHOST \
    -e PORT=_vaultkv1:/kv/pgprod:DBPORT \
    -e USER=_vaultkv1:/kv/pgprod:DBUSER \
    -e PASS=_vaultkv1:/kv/pgprod:DBPASSWD
```

### KV Version 2

The command below will mount a version 2 KV in the path `dbsecrets`
...
```sh
hoop admin create conn pgprod --type database/postgres -a <agent-name> \
    -e HOST=_vaultkv2:/dbsecrets/data/pgtest:DBHOST \
    -e PORT=_vaultkv2:/dbsecrets/data/pgtest:DBPORT \
    -e USER=_vaultkv2:/dbsecrets/data/pgtest:DBUSER \
    -e PASS=_vaultkv2:/dbsecrets/data/pgtest:DBPASSWD
```

<Tip>
You can specify the version of the secret as well by appending the suffix: `?version=:version-number`
</Tip>
