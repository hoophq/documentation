...
One-Time Access Requests require approval for each command before it executes. Unlike JIT Access Requests (which grant time-based access), One-Time requests give you command-level control:
...
**The key difference from JIT Access Requests:** One-Time requires approval for each command. JIT grants a time window where all commands are allowed.
...
### Step 1: Enable One-Time Access Requests
...
<CardGroup cols={2}>
  ...
  <Card title="Configuration Guide" icon="gear" href="/setup/configuration/access-requests/one-time-configuration">
