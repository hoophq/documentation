...
**Secrets Management** connects users to databases and servers without exposing credentials. Store passwords in a secrets provider and inject them at runtime:
...
When a user connects to a secrets-backed connection:
...
Credential rotation requires no connection config changes:
...
1. **Rotate monthly or quarterly** - Set a recurring schedule
