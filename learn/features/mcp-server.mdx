...
This skips OAuth entirely. Configure your MCP client with an `Authorization: Bearer <token>` header using a token from `hoop login`. See [Option A above](#option-a-bearer-token-always-available).
