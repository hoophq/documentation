...
A Sidecar that loses contact keeps serving traffic with the configuration it already holds, so a Control Plane restart is not an outage for the resources behind it. The exception is a listener that holds statements for human review: it files each held statement with the Control Plane, so while the Control Plane is unreachable, every statement that listener would hold is denied.
...
### Human review of held statements

An analyzer risk level set to `require_review` holds the statement for a person. The Sidecar files it with the Control Plane, and the listener's `approval_rule` names the access request rule that decides who may approve. The statement waits on its connection for up to 30 minutes, and an approval in that time runs it. After a timeout, an identical retry after approval goes through once. See [Holding a statement for a person](/setup/configuration/hoop-sidecar/risk-analysis#holding-a-statement-for-a-person).
