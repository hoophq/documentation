...
<Tabs>
  <Tab title="Local Installation">
    This process is recommended to run in your **local machine**.
...
=======
    Visit your instance at [http://127.0.0.1](http://127.0.0.1) and follow the next steps.
...
>>>>>>> Stashed changes
  ...
  <Tab title="Remote Installation">
    This process is recommended to run in a **remote machine**. The services will be available at `http(s)://<vm-public-dns>`.
    <Steps>
      ...
      <Step title="Signup to Developer's Portal">
        Visit your instance at `http://$HOOP_PUBLIC_HOSTNAME` and follow the next steps.
      </Step>
    </Steps>
  </Tab>
</Tabs>
...
<Tabs>
  <Tab title="Connect with your IDE">
    In this step, we'll connect to **PostgresSQL** using your favorite IDE via Hoop.
    ...
    <Steps>
      <Step title="Access your terminal and Install hoop.dev's CLI">
        ...
        2. Expose the environment variable containing the DNS or IP of your instance

        <Tabs>
          <Tab title="Local Installation">
            1. Expose the environment variable below
          ...
          <Tab title="Remote Installation">
            - Replace `<vm-public-dns>` with your remote machine DNS or IP
            ```sh
            export HOOP_PUBLIC_HOSTNAME=<vm-public-dns>
            ```

            <Note>
              The steps 2 and 3 are only required when running the gateway with TLS and self signed certificates.
            </Note>
            - Copy the output of the command below to your local machine (e.g.: `/tmp/ca.crt`)
            ```sh
            docker exec hoop-nginx-1 cat /hoopdata/tls/ca.crt
            ```

            - Export the environment variable containing the path of the CA
            ```sh
      ...
      <Step title="Configure and Login via CLI">
        ...
        - Configure the command line to access your gateway instance
        ```sh
  ...
  <Tab title="Access a Jump Host">
    In this step, we'll connect a distinct machine to act as a Jump Host in your private network.
    ...
    <Steps>
      ...
      <Step title="Obtain the Token in the Webapp">
        1. Visit the webapp URL at [http://127.0.0.1](http://127.0.0.1).
...
<Steps>
  <Step title="Access Zitadel admin panel">
    <Tabs>
      <Tab title="Local Installation">
        - [http://127.0.0.1/ui/console](http://127.0.0.1/ui/console)
      </Tab>
      <Tab title="Remote Installation">
        - `http://$HOOP_PUBLIC_HOSTNAME/ui/console`
      </Tab>
    </Tabs>
  </Step>
