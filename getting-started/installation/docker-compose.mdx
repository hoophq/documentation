...
<Steps>
  <Step title="Obtain the VM Public DNS or IP">
    ...
    <Warning>
    Make sure to open the inbound traffic to port **TCP/443**.
    </Warning>
  </Step>
  ...
  <Tab title="Connect with your IDE">
    ...
    <Steps>
      ...
      <Step title="Configure and Login via CLI">
        ...
        hoop config create --api-url https://$HOOP_PUBLIC_HOSTNAME --tls-ca /path/to/ca.crt

        hoop login
        ```

        <Tip>
          Make sure to replace `$HOOP_PUBLIC_HOSTNAME` with the VM public DNS.
  ...
  <Tab title="Access a Jump Host">
    ...
    <Steps>
      ...
      <Step title="Obtain the Token in the Webapp">
        1. Visit the webapp URL at **https://$HOOP_PUBLIC_HOSTNAME**.
      ...
      <Step title="Run the command to connect your Jump Host">
        ...
        2. Export the Certificate Authority file

        ```sh
        # asumming the file was saved at /tmp/ca.pem
        export HOOP_TLSCA=file:///tmp/ca.pem
        ```
      ...
      <Step title="Connect into your Jump Host">
        ...
        1. Open another terminal window and configure the cli
        ```sh
        hoop config create --api-url https://$HOOP_PUBLIC_HOSTNAME \
...
<Steps>
  <Step title="Access Zitadel admin panel">
    Go to `https://$HOOP_PUBLIC_HOSTNAME/ui/console`
  </Step>
