...
<Note>
Set `HOOP_SIDECAR_CONFIG` to an **empty** string to run against a Control Plane. A path naming a file that does not exist is an error, not a reason to fall through to the handshake. The [Helm chart](/install/kubernetes#control-plane) does this for you.
</Note>
...
If the lane talks to Envoy over a unix socket rather than a published port, both containers need to agree on the directory that carries it — see [Setting up the socket directory](/install/cli#setting-up-the-socket-directory).
